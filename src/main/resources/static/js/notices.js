// Auto-generated CRUD screen for Notices
requireAuth();

const ENDPOINT = "/api/notices";
const FIELDS = [{name:"title", label:"Title", type:"text"}, {name:"description", label:"Description", type:"text"}, {name:"date", label:"Date", type:"date"}, {name:"createdBy", label:"Created By", type:"text"}, {name:"priority", label:"Priority", type:"text"}];
...
async function loadRecords() {
  try {
    ...
    showAlert("Failed to load Notices: " + err.message, "danger");
  }
}
...
const COLUMNS = ["title", "date", "createdBy", "priority"];
...
function openAddModal() {
  editingId = null;
  document.getElementById("formModalTitle").textContent = "Add Notice";
  FIELDS.forEach(f => {
...
async function editRecord(id) {
  ...
  if (!item) return;
  document.getElementById("formModalTitle").textContent = "Edit Notice";
  FIELDS.forEach(f => {
    const el = document.getElementById("f_" + f.name);
