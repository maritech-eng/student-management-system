// Auto-generated CRUD screen for Library - Issue/Return
requireAuth();

const ENDPOINT = "/api/library/transactions";
const FIELDS = [{name:"bookId", label:"Book Id", type:"text"}, {name:"studentId", label:"Student Id", type:"text"}, {name:"issueDate", label:"Issue Date", type:"date"}, {name:"dueDate", label:"Due Date", type:"date"}, {name:"status", label:"Status", type:"text"}];
...
async function loadRecords() {
  try {
    ...
    showAlert("Failed to load Library - Issue/Return: " + err.message, "danger");
  }
}
...
const COLUMNS = ["bookId", "studentId", "issueDate", "dueDate", "status", "fine"];
...
function openAddModal() {
  editingId = null;
  document.getElementById("formModalTitle").textContent = "Add Transaction";
  FIELDS.forEach(f => {
...
async function editRecord(id) {
  ...
  if (!item) return;
  document.getElementById("formModalTitle").textContent = "Edit Transaction";
  FIELDS.forEach(f => {
    const el = document.getElementById("f_" + f.name);
