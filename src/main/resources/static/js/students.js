// Auto-generated CRUD screen for Students
requireAuth();

const ENDPOINT = "/api/students";
const FIELDS = [{name:"registerNumber", label:"Register Number", type:"text"}, {name:"name", label:"Name", type:"text"}, {name:"email", label:"Email", type:"text"}, {name:"phone", label:"Phone", type:"text"}, {name:"dob", label:"Dob", type:"date"}, {name:"gender", label:"Gender", type:"text"}, {name:"bloodGroup", label:"Blood Group", type:"text"}, {name:"address", label:"Address", type:"text"}, {name:"city", label:"City", type:"text"}, {name:"state", label:"State", type:"text"}, {name:"pincode", label:"Pincode", type:"text"}, {name:"departmentId", label:"Department Id", type:"text"}, {name:"courseId", label:"Course Id", type:"text"}, {name:"year", label:"Year", type:"number"}, {name:"semester", label:"Semester", type:"number"}, {name:"section", label:"Section", type:"text"}, {name:"admissionDate", label:"Admission Date", type:"date"}, {name:"guardianName", label:"Guardian Name", type:"text"}, {name:"guardianPhone", label:"Guardian Phone", type:"text"}, {name:"emergencyContact", label:"Emergency Contact", type:"text"}, {name:"photoUrl", label:"Photo Url", type:"text"}];
...
const COLUMNS = ["registerNumber", "name", "email", "departmentId", "courseId", "year", "section"];
...
document.getElementById("f_photoUrl_file")?.addEventListener("change", async (e) => {
  const file = e.target.files[0];
  if (!file) return;
  const formData = new FormData();
  formData.append("file", file);
  try {
    const res = await fetch("/api/upload/student-photo", { method: "POST", body: formData, credentials: "include" });
    const data = await res.json();
    if (res.ok) {
      document.getElementById("f_photoUrl").value = data.url;
