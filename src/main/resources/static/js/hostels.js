// Auto-generated CRUD screen for Hostels
requireAuth();

const ENDPOINT = "/api/hostel/hostels";
const FIELDS = [{name:"name", label:"Name", type:"text"}, {name:"block", label:"Block", type:"text"}];
...
async function loadRecords() {
  try {
    ...
    showAlert("Failed to load Hostels: " + err.message, "danger");
  }
}
...
function openAddModal() {
  editingId = null;
  document.getElementById("formModalTitle").textContent = "Add Hostel";
  FIELDS.forEach(f => {
...
async function editRecord(id) {
  ...
  if (!item) return;
  document.getElementById("formModalTitle").textContent = "Edit Hostel";
  FIELDS.forEach(f => {
    const el = document.getElementById("f_" + f.name);
...
async function saveEntity() {
  ...
  try {
    if (editingId) {
      ...
      showAlert("Hostel updated successfully", "success");
    } else {
