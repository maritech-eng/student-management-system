// Auto-generated CRUD screen for Hostel Allocations
requireAuth();

const ENDPOINT = "/api/hostel/allocations";
const FIELDS = [{name:"studentId", label:"Student Id", type:"text"}, {name:"roomId", label:"Room Id", type:"text"}];
...
async function loadRecords() {
  try {
    ...
    showAlert("Failed to load Hostel Allocations: " + err.message, "danger");
  }
}
...
const COLUMNS = ["studentId", "roomId", "allocationDate", "status"];
...
function openAddModal() {
  editingId = null;
  document.getElementById("formModalTitle").textContent = "Add Allocation";
  FIELDS.forEach(f => {
...
async function editRecord(id) {
  ...
  if (!item) return;
  document.getElementById("formModalTitle").textContent = "Edit Allocation";
  FIELDS.forEach(f => {
    const el = document.getElementById("f_" + f.name);
...
async function saveEntity() {
  ...
  try {
    if (editingId) {
      ...
      showAlert("Allocation added successfully", "success");
    }
