// Auto-generated CRUD screen for Attendance
requireAuth();

const ENDPOINT = "/api/attendance";
const FIELDS = [{name:"studentId", label:"Student Id", type:"text"}, {name:"subjectId", label:"Subject Id", type:"text"}, {name:"facultyId", label:"Faculty Id", type:"text"}, {name:"date", label:"Date", type:"date"}, {name:"semester", label:"Semester", type:"number"}, {name:"status", label:"Status", type:"text"}];
...
async function loadRecords() {
  try {
    ...
    showAlert("Failed to load Attendance: " + err.message, "danger");
  }
}
...
const COLUMNS = ["studentId", "subjectId", "date", "status"];
...
function openAddModal() {
  editingId = null;
  document.getElementById("formModalTitle").textContent = "Add Attendance Record";
  FIELDS.forEach(f => {
...
async function editRecord(id) {
  ...
  if (!item) return;
  document.getElementById("formModalTitle").textContent = "Edit Attendance Record";
  FIELDS.forEach(f => {
    const el = document.getElementById("f_" + f.name);
