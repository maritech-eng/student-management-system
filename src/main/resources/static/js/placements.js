// Auto-generated CRUD screen for Placements
requireAuth();

const ENDPOINT = "/api/placements";
const FIELDS = [{name:"companyName", label:"Company Name", type:"text"}, {name:"jobRole", label:"Job Role", type:"text"}, {name:"packageOffered", label:"Package Offered", type:"text"}, {name:"eligibility", label:"Eligibility", type:"text"}, {name:"driveDate", label:"Drive Date", type:"date"}, {name:"location", label:"Location", type:"text"}, {name:"description", label:"Description", type:"text"}];
...
async function loadRecords() {
  try {
    ...
    showAlert("Failed to load Placements: " + err.message, "danger");
  }
}
...
const COLUMNS = ["companyName", "jobRole", "packageOffered", "driveDate"];
...
function openAddModal() {
  editingId = null;
  document.getElementById("formModalTitle").textContent = "Add Placement Drive";
  FIELDS.forEach(f => {
...
async function editRecord(id) {
  ...
  if (!item) return;
  document.getElementById("formModalTitle").textContent = "Edit Placement Drive";
  FIELDS.forEach(f => {
    const el = document.getElementById("f_" + f.name);
...
async function saveEntity() {
  ...
  try {
    if (editingId) {
      ...
      showAlert("Placement Drive updated successfully", "success");
    } else {
      await apiPost(ENDPOINT, payload);
      showAlert("Placement Drive added successfully", "success");
    }
