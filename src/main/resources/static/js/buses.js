// Auto-generated CRUD screen for Transport - Buses
requireAuth();

const ENDPOINT = "/api/transport/buses";
const FIELDS = [{name:"busNumber", label:"Bus Number", type:"text"}, {name:"driverName", label:"Driver Name", type:"text"}, {name:"driverPhone", label:"Driver Phone", type:"text"}, {name:"route", label:"Route", type:"text"}, {name:"pickupPoint", label:"Pickup Point", type:"text"}, {name:"capacity", label:"Capacity", type:"number"}];
...
async function loadRecords() {
  try {
    ...
    showAlert("Failed to load Transport - Buses: " + err.message, "danger");
  }
}
...
const COLUMNS = ["busNumber", "driverName", "route", "capacity"];
...
function openAddModal() {
  editingId = null;
  document.getElementById("formModalTitle").textContent = "Add Bus";
  FIELDS.forEach(f => {
...
async function editRecord(id) {
  ...
  if (!item) return;
  document.getElementById("formModalTitle").textContent = "Edit Bus";
  FIELDS.forEach(f => {
    const el = document.getElementById("f_" + f.name);
